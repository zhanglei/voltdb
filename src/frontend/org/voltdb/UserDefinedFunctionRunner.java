 ...
 */
public abstract class UserDefinedFunctionRunner {
    ...
    public UserDefinedFunctionRunner(String functionName, int functionId, Object funcInstance) {
        m_functionName = functionName;
        m_functionId = functionId;
        m_functionInstance = funcInstance;
        String logMsg = String.format("Creating the user-defined function runner for %s (ID = %s)",
                m_functionName, m_functionId);
        s_logger.debug(logMsg);
    }

    static byte[] readVarbinary(ByteBuffer buffer) {
        // Sanity check the size against the remaining buffer size.
        if (VAR_LEN_SIZE > buffer.remaining()) {
            String errMsg = String.format("Can't read varbinary size as %d byte integer from buffer with %d bytes remaining.",
    ...
    static Object getValueFromBuffer(ByteBuffer buffer, VoltType type) {
        switch (type) {
        case TINYINT:
            return buffer.get();
        case SMALLINT:
